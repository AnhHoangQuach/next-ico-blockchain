...
export default function MarketView() {
  ...
  const [modalType, setModalType] = React.useState<'LISTING' | 'AUCTION'>('LISTING')
  ...
  const getListNft = React.useCallback(async () => {
    ...
    const myAuctions = auctionNfts.filter((p) => p.auctioneer === wallet.address)
  ...
  return (
    <Flex w="full">
              ...
              {auctions.map((nft, index) => (
                <NftAuction
                  ...
                  onAction={async (a) => {
                    ...
                    try {
                      const auctionContract = new AuctionContract(web3Provider)
                      const tx = await auctionContract.cancelAuction(nft.auctionId)
                      setTxHash(tx)
                      onOpenSuccess()
                      await getListNft()
                    } catch (ex) {
      ...
      <SuccessModal hash={txHash} title="SUCCESS" isOpen={isSuccess} onClose={onCloseSuccess} />
    </Flex>
  )
}
