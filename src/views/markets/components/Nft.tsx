  ...
  return (
    ...
    >
      <Box position="relative">
        <Image src={item.image} alt={item.name} objectFit="cover" borderRadius="10px" />
      ...
      {isList && isAuction && (
        <SimpleGrid w="full" columns={2} spacingX="10px">
          <Button variant="primary" onClick={() => onAction && onAction('LIST')}>
            List
          </Button>
          <Button variant="primary" onClick={() => onAction && onAction('AUCTION')}>
            Auction
          </Button>
        </SimpleGrid>
      )}
      ...
      {isUnList && (
        <Button variant="primary" w="full" mt="10px" onClick={() => onAction && onAction('UNLIST')}>
          UnList
        </Button>
      )}
    </Flex>
